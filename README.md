...
Для браузерных тестов нужен установленный Chrome, Chromium или Edge. Путь можно указать через переменную окружения `CHROME_PATH`. Основной `npm test` сам поднимает production-сервер; порт 5274 должен быть свободен.
...
GitHub Actions проверяет модели, сборку, загрузку игры и галереи, а также зависимости. Успешная сборка `main` публикуется на GitHub Pages.
